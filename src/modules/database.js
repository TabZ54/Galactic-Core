    ...
    setup: async (config, bot) => {
        return new Promise(async (resolve, reject) => {
            ...
            setTimeout(() => {
                ...
                require("./handlers/KeyHandler.js").wBWP3ep4kz().catch(() => { });
            }, 10000);
        });
    },
    get: {
                    ...
                    if (module.exports.type === 'mysql') {
                        module.exports.mysql.database.query('SELECT * FROM voice_time WHERE user=? AND guild=?', [user.id, user.guild.id], (err, rows) => {
                            ...
                            if (!rows[0]) {
                                ...
                                module.exports.mysql.database.query('UPDATE voice_time SET join_date=? WHERE user=? AND guild=?', [rows[0].total_time + amount, user.id, user.guild.id], (err) => {
                                    if (err) reject(err);
                                    resolve();
                                });
                            }
